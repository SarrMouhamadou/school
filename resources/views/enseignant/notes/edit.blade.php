        ...
        <form action="{{ route('enseignant.notes.update', $note->id) }}" method="POST">
                ...
                <select name="matiere_id" id="matiere_id"
                    ...
                    @foreach ($matieres as $matiere)
                        <option value="{{ $matiere->id }}"
                            {{ old('matiere_id', $note->matiere_id) == $matiere->id ? 'selected' : '' }}>{{ $matiere->nom }}
                            (Classe: {{ $matiere->pivot->classe->nom }})</option>
                    @endforeach
                </select>
                @error('matiere_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <select name="classe_id" id="classe_id"
                    ...
                    @foreach ($classes as $classe)
                        <option value="{{ $classe->id }}"
                            {{ old('classe_id', $note->classe_id) == $classe->id ? 'selected' : '' }}>{{ $classe->nom }}
                        </option>
                    @endforeach
                </select>
                @error('classe_id')
            ...
            <div class="mb-4">
                ...
                <select name="semestre" id="semestre"
                    ...
                    <option value="1" {{ old('semestre', $note->semestre) == '1' ? 'selected' : '' }}>Semestre 1
                    </option>
                    <option value="2" {{ old('semestre', $note->semestre) == '2' ? 'selected' : '' }}>Semestre 2
                    </option>
                </select>
                @error('semestre')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="type_evaluation" id="type_evaluation"
                    ...
                    <option value="devoir"
                        {{ old('type_evaluation', $note->type_evaluation) == 'devoir' ? 'selected' : '' }}>Devoir</option>
                    <option value="examen"
                        {{ old('type_evaluation', $note->type_evaluation) == 'examen' ? 'selected' : '' }}>Examen</option>
                </select>
