...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <a href="{{ route('admin.matieres.create') }}"
            class="bg-blue-500 text-white px-4 py-2 rounded mb-4 inline-block">Ajouter une matière</a>
        ...
        <table class="w-full border-collapse border">
            ...
            <tbody>
                @forelse ($matieres as $matiere)
                    <tr>
                        <td class="border p-2">{{ $matiere->id }}</td>
                        <td class="border p-2">{{ $matiere->nom }}</td>
                        <td class="border p-2">{{ $matiere->coefficient }}</td>
                        <td class="border p-2">
                            <a href="{{ route('admin.matieres.show', $matiere->id) }}"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="border p-2 text-center">Aucune matière trouvée.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
